...
function DataStorePages:GetCurrentPage()
	return Promise.Defer(function(resolve, reject)
		...
		(success and resolve or reject)(page)
	end)
end
...
function Data.new(name, scope, ordered)
	...
	assert(ordered == nil or type(ordered) == "boolean", "Argument #3 (ordered) must be a boolean or nil")
...
function Data:Get(key, defaultVal)
	...
	return self:_load(key):AndThen(function(value)
		if value == nil and defaultVal ~= nil then
			...
			return self:Set(key, value):AndThen(function()
				return value
			end)
		else
			return value
...
function Data:Set(key, value)
	...
	return self:_loadIfNotCached(key):AndThen(function()
		if value == nil then
...
function Data:Increment(key, increment)
	...
	return self:Get(key, 0):AndThen(function(value)
		if type(value) ~= "number" then
			return error("Cannot increment a non-number value")
		end
...
function Data:Destroy(save)
	...
	return savePromise:AndThen(function()
...
function Data:Start()
	...
	Players.PlayerRemoving:Connect(function(player)
		...
		Promise.Delay(self.PlayerLeftSaveInterval):Await()
	...
	HeartbeatSpawn(function()
		while 1 do
			Promise.Delay(self.AutoSaveInterval):Await()
			if gameClosing then
				break
			end
