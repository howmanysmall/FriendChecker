...
local function bitBuffer(stream)
	...
	local function writeUnsigned(width, n)
		...
		assert(type(n) == "number", "argument #2 to BitBuffer.writeUnsigned should be a number (got " .. typeof(n) .. " " .. n .. ")")
	...
	local function writeUInt32(n)
		...
		assert(n >= 0 and n <= 4294967295, "argument #1 to BitBuffer.writeUInt32 should be in the range [0, 4294967295] (got " .. n .. ")")
	...
	local function writeBoolean(value)
		assert(type(value) == "boolean", "argument #1 to BitBuffer.writeBoolean should be a boolean")
		writeByte(value == true and 1 or 0)
	end
